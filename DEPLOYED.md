...
- เวอร์ชันแคช Service Worker ปัจจุบัน: **`pkchem-v26`** (ใน `sw.js`) — เลขท้ายหน้าเว็บ (`APP_VERSION` ใน `index.html`) ตั้งเป็น v26 ไว้ให้ตรงกัน (ไม่บังคับต้องอัปทุกครั้ง แค่ใช้ดูว่าโหลดรุ่นไหน)
...
4. **สำคัญมาก:** ทุกครั้งที่แก้เนื้อหา ให้แก้ไฟล์ `sw.js` ด้วย — เปลี่ยนเลขเวอร์ชันบรรทัดที่ 2 ขึ้นทีละหนึ่ง
   ```js
   const CACHE = 'pkchem-v26';   →   const CACHE = 'pkchem-v27';
   ```
